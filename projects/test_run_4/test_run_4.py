...
EXPERIMENTS = [
    (1, [f"yamls/ex{i}.yaml" for i in range(1, 5)]),
    (2, [f"yamls/ex{i}.yaml" for i in range(1, 5)]),
    (4, [f"yamls/ex{i}.yaml" for i in range(1, 5)])
]
...
with LOG.open("w", newline="") as f:
    ...
    for batch, yamls in EXPERIMENTS:
        ...
        # Шаг 1: preprocess
        subprocess.run(["python3","projects/test_run_4/s1_inputs.py","--yamls", *yamls,"--out_dir",out_dir], check=True)

        # Шаг 2: structure
        t0 = time.time()
        subprocess.run(["python3","projects/test_run_4/s2_structure.py","-b",str(batch),"--out_dir",out_dir], check=True)
        t1 = time.time()

        # Шаг 3: affinity
        t2 = time.time()
        subprocess.run(["python3","projects/test_run_4/s3_affinity.py","-b",str(batch),"--out_dir",out_dir], check=True)
        t3 = time.time()
